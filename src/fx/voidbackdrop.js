 ...
 *
 * 2. THE BANDS ARE TWO, AND THEY ARE DISTANCES, NOT DECORATION. Near backdrop
 *    (470-600 m) still resolves a silhouette with facets in it; mid (690-850 m)
 *    reads as shape without value. Both are real geometry and are authored in
 *    `BANDS`. There WAS a third, at 780-1010 m, and it is deleted — the block
 *    headed "THE FAR BAND IS GONE" below is the argument, and the short form is
 *    that `public/sky/void-dome.png` does the far distance better than any
 *    number of prisms could. The two brightness rungs that remain are the
 *    mechanism; see the note on `makeMaterial`.
 ...
 *
 * 3. IT IS BACKDROP, SO IT COSTS BACKDROP MONEY. Two draw calls, two
 *    InstancedMeshes of prisms, and about 22k triangles for the whole layer.
 *    LOD is by band and is decided at BUILD time rather than per frame: a thing
 *    that can never be approached can never need a closer mesh. No runtime
...
const BANDS = [
  ...
  // THE FAR BAND IS NO LONGER HERE AT ALL. See the block immediately below.
]
...
/* ==========================================================================
 * THE FAR BAND IS GONE, AND THAT IS THE POINT.
 * ==========================================================================
 *
 * It was 306 extruded prisms, then ~720 camera-facing cards baked from those
 * same prisms into an 80-tile atlas — the impostor machinery that used to fill
 * the next 650 lines of this file. Ethan, looking at the build it produced:
 *
 *   "honestly the random shapes in the background is very weak hoping the
 *    image method will improve it."
 *
 * He is right, and the reason is structural rather than a matter of tuning.
 * Every shape in that layer came from `ruinGeometry()` above — a tapered n-gon
 * prism with a broken point. Bake ten of them into a tile and you get an
 * angular blob; draw seven hundred tiles and you get seven hundred angular
 * blobs. There is no count at which prisms become a cathedral, so the density
 * dial was the only dial the layer had, and density for its own sake is
 * precisely what he called weak.
 *
 * `public/sky/void-dome.png` is a painting with real gothic architecture in it
 * — arches, tracery, buttressed spires — and it wins on the only axis that
 * matters at 900 m, which is what the silhouette actually is. Measured side by
 * side at `summit` and `plunge` with the dome wired in, the cards did not add
 * to it: they drew a bed of pale faceted gravel ACROSS the painted architecture
 * and hid it. Deleting them made the frame better, not merely cheaper.
 *
 * WHAT WENT WITH THEM, honestly stated:
 *
 *  - One draw call, ~1 440 triangles, and about 42 ms of atlas bake at boot.
 *  - A 2016 x 2016 RGBA render target (~16 MB) held for the life of the level.
 *  - The far-plane NDC squash in the card vertex shader, which existed to stop
 *    the outermost cards being clipped. Nothing is out there to clip now.
 *  - Real parallax at 780-1010 m. This is the genuine loss and it should not be
 *    glossed: a painted dome at infinity does not shift as the player climbs,
 *    and the cards did. What remains carrying parallax is the near band
 *    (470-600 m) and the mid band (690-850 m), both real geometry, both moving
 *    correctly. The judgement is that two bands of moving silhouette plus a
 *    painting that is actually architecture beats three bands where the third
 *    is moving gravel.
 *
 * If a third band is ever wanted again, it should not be prisms. `git log` has
 * the impostor implementation in full if the atlas approach is ever worth
 * re-applying to a better source shape.
 */
...
export class VoidBackdrop {
   ...
   * @param {THREE.WebGLRenderer} [renderer] accepted and unused since the far
   *   band was deleted (see the block above). Kept in the signature because
   *   world.js and the headless tools both pass it and a silently-changed arity
   *   is a worse trade than an unused parameter with a comment on it.
   */
  ...
  constructor(scene, theme, renderer = null) {
     ...
     * curve cannot put two bands at two deliberate value rungs — it produces a
     * monotone ramp in one direction, which is a fog bank.
     * Two materials cost nothing extra (they are the same program with
    ...
    const makeMaterial = (haze) => {
      const m = new THREE.ShaderMaterial({
        ...
        // each other, and the whole point of separate bands is that the near
        // one OCCLUDES the one behind it.
        transparent: false,
        depthWrite: true,
        depthTest: true,
    ...
    for (const { band, items } of bands) {
      ...
      // AFTER the sky dome (renderOrder 0, and it writes no depth), so a band
      // is not clipped by the dome sphere's own 900 m radius — the mid band's
      // outer edge sits past it on purpose. Before the additive layers, at 3.
      mesh.renderOrder = 1
